...
class SerperBaseHTTP {
  ...
  protected async request<T>(
    ...
    if (!this.apiKey) {
      throw new Error(
        `SERPER_SEARCH_API_KEY is required for ${this.provider} ${variant} search.`,
      );
    }
    ...
    if (!response.ok) {
      ...
      const err: any = new Error(
        `Serper ${this.provider} ${variant} search failed: ${response.status} ${response.statusText} ${text}`.trim(),
      );
  ...
  async imageSearch(query: SerperSearchQueryParams) {
    const parsed = await this.request<SerperImageSearchResponse>(
      "images",
      query,
    );
