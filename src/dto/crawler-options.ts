...
export const IMAGE_RETENTION_MODES = [
  "none",
  "all",
  "alt",
  "all_p",
  ...
  openapi: {
    operation: {
      parameters: {
        Accept: {
          description:
            `Specifies your preference for the response format.\n\n` +
            `Supported formats: \n` +
            `- text/event-stream\n` +
        ...
        "X-Respond-With": {
          description:
            `Specifies the (non-default) form of the crawled data you prefer.\n\n` +
            `Supported formats: \n` +
            `- markdown\n` +
            `- html\n` +
            `- text\n` +
        ...
        "X-Wait-For-Selector": {
          description:
            `Specifies a CSS selector to wait for the appearance of such an element before returning.\n\n` +
        ...
        "X-Target-Selector": {
          description:
            `Specifies a CSS selector for return target instead of the full html.\n\n` +
        ...
        "X-Remove-Selector": {
          description:
            `Specifies a CSS selector to remove elements from the full html.\n\n` +
        ...
        "X-Keep-Img-Data-Url": {
          description:
            `Keep data-url as it instead of transforming them to object-url. (Only applicable when targeting markdown format)\n\n` +
        ...
        "X-Proxy-Url": {
          description:
            `Specifies your custom proxy if you prefer to use one.\n\n` +
            `Supported protocols: \n` +
            `- http\n` +
            `- https\n` +
            `- socks4\n` +
        ...
        "X-Set-Cookie": {
          description:
            `Sets cookie(s) to the headless browser for your request. \n\n` +
        ...
        "X-With-Generated-Alt": {
          description:
            `Enable automatic alt-text generating for images without an meaningful alt-text.\n\n` +
        ...
        "X-Token-Budget": {
          description:
            "Specify a budget in tokens.\n\nIf the resulting token cost exceeds the budget, the request is rejected.",
          in: "header",
          schema: { type: "string" },
        },
        "X-Respond-Timing": {
          description:
            `Explicitly specify the respond timing. One of the following:\n\n` +
        ...
        "X-Engine": {
          description:
            "Specify the engine to use for crawling.\n\nSupported: browser, direct, cf-browser-rendering",
          in: "header",
          schema: { type: "string" },
        },
        "X-Base": {
          description:
            "Select base modes of relative URLs.\n\nSupported: initial, final",
          in: "header",
          schema: { type: "string" },
        },
        "X-Md-Heading-Style": {
          description:
            "Heading style of the generated markdown.\n\nThis is an option passed through to [Turndown](https://github.com/mixmark-io/turndown?tab=readme-ov-file#options).\n\nSupported: setext, atx",
          in: "header",
          schema: { type: "string" },
        },
        "X-Md-Hr": {
          description:
            "Hr text of the generated markdown.\n\nThis is an option passed through to [Turndown](https://github.com/mixmark-io/turndown?tab=readme-ov-file#options).",
          in: "header",
          schema: { type: "string" },
        },
        "X-Md-Bullet-List-Marker": {
          description:
            "Bullet list marker of the generated markdown.\n\nThis is an option passed through to [Turndown](https://github.com/mixmark-io/turndown?tab=readme-ov-file#options).\n\nSupported: -, +, *",
          in: "header",
          schema: { type: "string" },
        },
        "X-Md-Em-Delimiter": {
          description:
            "Em delimiter of the generated markdown.\n\nThis is an option passed through to [Turndown](https://github.com/mixmark-io/turndown?tab=readme-ov-file#options).\n\nSupported: _, *",
          in: "header",
          schema: { type: "string" },
        },
        "X-Md-Strong-Delimiter": {
          description:
            "Strong delimiter of the generated markdown.\n\nThis is an option passed through to [Turndown](https://github.com/mixmark-io/turndown?tab=readme-ov-file#options).\n\nSupported: **, __",
          in: "header",
          schema: { type: "string" },
        },
        "X-Md-Link-Style": {
          description:
            "Link style of the generated markdown.\n\nThis is an option passed through to [Turndown](https://github.com/mixmark-io/turndown?tab=readme-ov-file#options).\n\nSupported: inlined, referenced, discarded",
          in: "header",
          schema: { type: "string" },
        },
        "X-Md-Link-Reference-Style": {
          description:
            "Link reference style of the generated markdown.\n\nThis is an option passed through to [Turndown](https://github.com/mixmark-io/turndown?tab=readme-ov-file#options).\n\nSupported: full, collapsed, shortcut, discarded",
          in: "header",
          schema: { type: "string" },
        },
      },
...
export class CrawlerOptions extends AutoCastable {
  ...
  static override from(input: any) {
    ...
    const customMode =
      ctx?.get("x-respond-with") || ctx?.get("x-return-format");
    if (customMode) {
    ...
    if (instance.respondWith?.includes("lm")) {
      if (
        instance.respondWith.includes("content") ||
        instance.respondWith.includes("markdown")
      ) {
    ...
    const removeSelector = ctx
      ?.get("x-remove-selector")
      ?.split(", ")
      .filter(Boolean);
    instance.removeSelector ??= removeSelector?.length
      ? removeSelector
      : undefined;
    const targetSelector = ctx
      ?.get("x-target-selector")
      ?.split(", ")
      .filter(Boolean);
    instance.targetSelector ??= targetSelector?.length
      ? targetSelector
      : undefined;
    const waitForSelector = ctx
      ?.get("x-wait-for-selector")
      ?.split(", ")
      .filter(Boolean);
    instance.waitForSelector ??=
      (waitForSelector?.length ? waitForSelector : undefined) ||
      instance.targetSelector;
    ...
    if (withIframe) {
      instance.withIframe =
        withIframe.toLowerCase() === "quoted" ? "quoted" : Boolean(withIframe);
    }
    ...
    const setCookieHeaders = (
      ctx?.get("x-set-cookie")?.split(", ") ||
      (instance.setCookies as any as string[])
    ).filter(Boolean);
  ...
  isSnapshotAcceptableForEarlyResponse(snapshot: PageSnapshot) {
    ...
    if (
      presumedTiming === RESPOND_TIMING.MEDIA_IDLE &&
      snapshot.lastMediaResourceLoaded &&
      snapshot.lastMutationIdle
    ) {
      const now = Date.now();
      if (
        Math.max(
          snapshot.lastMediaResourceLoaded,
          snapshot.lastContentResourceLoaded || 0,
        ) +
          500 <
        now
      ) {
    ...
    if (
      (this.respondWith.includes("vlm") ||
        this.respondWith.includes("pageshot")) &&
      !snapshot.pageshot
    ) {
      return false;
    }
    if (
      (this.respondWith.includes("vlm") ||
        this.respondWith.includes("screenshot")) &&
      !snapshot.screenshot
    ) {
      return false;
    }
    if (
      presumedTiming === RESPOND_TIMING.RESOURCE_IDLE &&
      snapshot.lastContentResourceLoaded &&
      snapshot.lastMutationIdle
    ) {
      const now = Date.now();
    ...
    if (
      presumedTiming === RESPOND_TIMING.VISIBLE_CONTENT &&
    ...
    if (
      presumedTiming === RESPOND_TIMING.MUTATION_IDLE &&
      snapshot.lastMutationIdle
    ) {
      return true;
    }
  ...
  browserIsNotRequired() {
    ...
    if (
      this.respondWith.includes(CONTENT_FORMAT.PAGESHOT) ||
      this.respondWith.includes(CONTENT_FORMAT.SCREENSHOT)
    ) {
      return false;
    }
    if (this.injectFrameScript?.length || this.injectPageScript?.length) {
