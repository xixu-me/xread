...
const disallowedSummaryLinkSchemes = [
  "file:",
  "javascript:",
  "data:",
  "vbscript:",
...
function hasDisallowedSummaryLinkScheme(href: string) {
  ...
  return disallowedSummaryLinkSchemes.some((scheme) =>
...
export class SnapshotFormatter extends AsyncService {
  ...
  async formatSnapshot(
    mode: string | "markdown" | "html" | "text" | "screenshot" | "pageshot",
    snapshot: PageSnapshot & {
      screenshotUrl?: string;
      pageshotUrl?: string;
    },
    nominalUrl?: URL,
    urlValidMs = 3600 * 1000 * 4,
  ) {
    const t0 = Date.now();
    const f = {
    ...
    if (mode.includes("screenshot")) {
      ...
      if (snapshot.screenshot && !snapshot.screenshotUrl) {
        ...
        snapshot.screenshotUrl =
          await this.firebaseObjectStorage.signDownloadUrl(
            fid,
            Date.now() + urlValidMs,
          );
      }
      ...
      Object.defineProperty(f, "textRepresentation", {
    ...
    if (mode.includes("pageshot")) {
      ...
      if (snapshot.pageshot && !snapshot.pageshotUrl) {
        ...
        snapshot.pageshotUrl = await this.firebaseObjectStorage.signDownloadUrl(
          fid,
          Date.now() + urlValidMs,
        );
      }
    ...
    if (
      snapshot.pdfs?.length &&
      (!snapshot.title || snapshot.title.startsWith("cache:"))
    ) {
      const pdf = await this.pdfExtractor.cachedExtract(
        snapshot.pdfs[0],
      ...
      if (pdf) {
        ...
        snapshot.parsed = {
          ...
          publishedTime: this.pdfExtractor
            .parsePdfDate(pdf.meta?.ModDate || pdf.meta?.CreationDate)
            ?.toISOString(),
        };
      }
    }
    ...
    if (
      ...
      this.logger.debug(`Formatting took ${dt}ms`, {
        mode,
    ...
    const imgDataUrlToObjectUrl = !Boolean(
      this.threadLocal.get("keepImgDataUrl"),
    );
    ...
    do {
      ...
      if (
        ...
        this.logger.warn("Degrading to text to protect the server", {
          url: snapshot.href,
          elemDepth: snapshot.maxElemDepth,
          elemCount: snapshot.elemCount,
        });
      ...
      const imageRetention = this.threadLocal.get(
        "retainImages",
      ) as CrawlerOptions["retainImages"];
      let imgIdx = 0;
      ...
      const customRules: { [k: string]: Rule } = {
        "img-retention": {
          ...
          replacement: (_content: string, node: HTMLElement) => {
            ...
            const maybeSrcSet: string = (
              node.getAttribute("srcset") || ""
            ).trim();
            if (!linkPreferredSrc && maybeSrcSet) {
              linkPreferredSrc = maybeSrcSet
                .split(",")
                .map((x) => x.trim())
                .filter(Boolean)[0];
            }
            ...
            const keySrc = (
              originalSrc.startsWith("data:")
                ? this.dataUrlToBlobUrl(originalSrc, snapshot.rebase)
                : src
            ).trim();
            const mapped = urlToAltMap[keySrc];
            const imgSerial = ++imgIdx;
            const idxArr = imageIdxTrack.has(keySrc)
              ? imageIdxTrack.get(keySrc)!
              : [];
            ...
            if (imgDataUrlToObjectUrl) {
              return alt
                ? `![Image ${imgSerial}: ${alt}](${keySrc})`
                : `![Image ${imgSerial}](${keySrc})`;
            }

            return alt
              ? `![Image ${imgSerial}: ${alt}](${src})`
              : `![Image ${imgSerial}](${src})`;
          },
        } as Rule,
      };
      ...
      const jsDomElementOfHTML = this.jsdomControl.snippetToElement(
        snapshot.html,
        snapshot.href,
      );
      ...
      if (!mode.includes("markdown") && snapshot.parsed?.content) {
        const jsDomElementOfParsed = this.jsdomControl.snippetToElement(
          snapshot.parsed.content,
          snapshot.href,
        );
        const par1 = this.jsdomControl.runTurndown(
          turnDownService,
          jsDomElementOfHTML,
        );
        imgIdx = 0;
        const par2 = snapshot.parsed.content
          ? this.jsdomControl.runTurndown(turnDownService, jsDomElementOfParsed)
          : "";
      ...
      if (!noGFMOpts) {
        turnDownService = turnDownService.use(
          noGFMOpts === "table" ? this.gfmNoTable : this.gfmPlugin,
        );
      }
      ...
      if (snapshot.imgs?.length && imageRetention?.endsWith("_p")) {
        const tasks = _.uniqBy(snapshot.imgs || [], "src").map(async (x) => {
          const r = await this.altTextService
            .getAltText(x)
            .catch((err: any) => {
              this.logger.warn(`Failed to get alt text for ${x.src}`, {
          ...
          if (r && x.src) {
            ...
            const keySrc = (
              x.src.startsWith("data:")
                ? this.dataUrlToBlobUrl(x.src, snapshot.rebase)
                : x.src
            ).trim();
      ...
      if (toBeTurnedToMd) {
        try {
          contentText = this.jsdomControl
            .runTurndown(turnDownService, toBeTurnedToMd)
            .trim();
          imgIdx = 0;
        } catch (err) {
          this.logger.warn(`Turndown failed to run, retrying without plugins`, {
            err,
          });
          const vanillaTurnDownService = this.getTurndown({ ...optsMixin });
          try {
            contentText = this.jsdomControl
              .runTurndown(vanillaTurnDownService, toBeTurnedToMd)
              .trim();
            imgIdx = 0;
          } catch (err2) {
            this.logger.warn(`Turndown failed to run, giving up`, {
              err: err2,
            });
          }
        }
      ...
      if (
        ...
        try {
          contentText = this.jsdomControl
            .runTurndown(turnDownService, jsDomElementOfHTML)
            .trim();
          imgIdx = 0;
        } catch (err) {
          this.logger.warn(`Turndown failed to run, retrying without plugins`, {
          ...
          try {
            contentText = this.jsdomControl
              .runTurndown(vanillaTurnDownService, jsDomElementOfHTML)
              .trim();
            imgIdx = 0;
          } catch (err2) {
            this.logger.warn(`Turndown failed to run, giving up`, {
    ...
    if (this.threadLocal.get("withImagesSummary")) {
      formatted.images = _(imageSummary)
        ...
        .map(([url, alt], i) => {
          ...
          const tag = idxTrack?.length
            ? `Image ${_.uniq(idxTrack).join(",")}`
            : `Hidden Image ${i + 1}`;
    ...
    if (this.threadLocal.get("withLinksSummary")) {
      ...
      if (this.threadLocal.get("withLinksSummary") === "all") {
        ...
        formatted.links = _(links)
          .filter(([_label, href]) => !hasDisallowedSummaryLinkScheme(href))
          .uniqBy(1)
          .fromPairs()
          .value();
      }
    ...
    if (countGPTToken(formatted.content) < 200) {
      ...
      if (
        ...
        snapshot.html.includes("cf-turnstile-response")
      ) {
        const msg =
...
${suffixMixins.length ? `\n${suffixMixins.join("\n\n")}\n` : ""}`;
  ...
  dataUrlToBlobUrl(dataUrl: string, baseUrl: string = "http://localhost/") {
    ...
    const mappedUrl = new URL(
      `blob:${refUrl.origin || "localhost"}/${md5Hasher.hash(dataUrl)}`,
    );
      ...
      for (const img of inferred.imgs) {
        ...
        const keySrc = (
          img.src.startsWith("data:")
            ? this.dataUrlToBlobUrl(img.src, snapshot.rebase)
            : img.src
        ).trim();
        const idxArr = imageIdxTrack.has(keySrc)
          ? imageIdxTrack.get(keySrc)!
  ...
  getTurndown(options?: {
    ...
    turnDownService.addRule("improved-heading", {
      ...
      replacement: (content, node, options) => {
        ...
        if (options.headingStyle === "setext" && hLevel < 3) {
          const underline = _.repeat(
            hLevel === 1 ? "=" : "-",
            Math.min(128, content.length),
          );
          return "\n\n" + content + "\n" + underline + "\n\n";
        } else {
    ...
    let realLinkStyle:
      ...
      | "discarded" = "inlined";
    if (
      turndownOpts?.linkStyle === "referenced" ||
      turndownOpts?.linkReferenceStyle
    ) {
    ...
    turnDownService.addRule("improved-code", {
      ...
      replacement: function (inputContent: any) {
        ...
        return (
          ...
          extraSpace +
          content +
          (delimiter === "```" && !content.endsWith(extraSpace)
            ? extraSpace
            : "") +
          delimiter
        );
  ...
  isPoorlyTransformed(content?: string, node?: Element) {
    ...
    if (
      ...
      if (node && tableElms.length) {
        const wrappingTables = _.without(
          tableElms,
          ...Array.from(deepTableElms || []),
        );
        const tableTextsLength = _.sum(
          wrappingTables.map((x) => x.innerHTML?.length || 0),
        );
  ...
  async createSnapshotFromFile(
    ...
    const contentType: string = (
      overrideContentType || (await file.mimeType)
    ).toLowerCase();
    ...
    try {
      ...
      if (contentType.startsWith("text/html")) {
        ...
        innerCharset ??= peek
          .match(/<meta[^>]+text\/html;\s*?charset=([^>"]+)/i)?.[1]
          ?.toLowerCase();
        innerCharset ??= peek
      ...
      throw new DataStreamBrokenError(
        `Failed to access ${url}: ${err?.message}`,
      );
    }

    throw new AssertionFailureError(
      `Failed to access ${url}: unexpected type ${contentType}`,
    );
  }
}
