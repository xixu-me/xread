...
const nodeCmapUrl =
  path.resolve(require.resolve("pdfjs-dist"), "../../cmaps") + "/";
...
function isRotatedByAtLeast35Degrees(
...
export class PDFExtractor extends AsyncService {
  ...
  async extract(url: string | URL) {
    ...
    for (const pageTextItems of textItems) {
      ...
      for (const textItem of pageTextItems) {
        if (textItem.height > articleAvgHeight + 3 * articleStdDevHeight) {
          ...
          textItem.height >
          articleAvgHeight + 2 * articleStdDevHeight
        ) {
          mode = "h2";
        } else if (
          textItem.height &&
          textItem.height < avgHeight - stdDevHeight
        ) {
          mode = "appendix";
    ...
    for (const x of mdOps) {
      ...
      const isNewStart =
        x.mode !== "space" &&
        (x.op === "new" ||
          (previousMode === "appendix" && x.mode !== previousMode));
  ...
  async cachedExtract(
    url: string,
    cacheTolerance: number = 1000 * 3600 * 24,
    ...
    const cache: PDFContent | undefined = nameUrl.startsWith("blob:")
      ? undefined
      : (
          await PDFContent.fromFirestoreQuery(
            PDFContent.COLLECTION.where("urlDigest", "==", digest)
    ...
    if (cache) {
      ...
      this.logger.info(
        `${stale ? "Stale cache exists" : "Cache hit"} for PDF ${nameUrl}, normalized digest: ${digest}, ${age}ms old, tolerance ${cacheTolerance}ms`,
        {
          data: url,
          url: nameUrl,
          digest,
      ...
      if (!stale) {
        ...
        try {
          const r = await this.firebaseObjectStorage.downloadFile(
            `pdfs/${cache._id}`,
          );
          ...
          this.logger.warn(`Unable to load cached content for ${nameUrl}`, {
            err,
          });
    ...
    try {
      ...
      this.logger.warn(`Unable to extract from pdf ${nameUrl}`, {
        err,
        url,
        nameUrl,
      });
      throw new AssertionFailureError(
        `Unable to process ${nameUrl} as pdf: ${err?.message}`,
      );
    }
    ...
    if (!this.asyncLocalContext.ctx.DNT && !nameUrl.startsWith("blob:")) {
      ...
      await this.firebaseObjectStorage.saveFile(
        `pdfs/${theID}`,
        Buffer.from(JSON.stringify(extracted), "utf-8"),
      ...
      PDFContent.save(
        ...
        this.logger.warn(`Unable to cache PDF content for ${nameUrl}`, {
          err: r,
        });
      });
    }
