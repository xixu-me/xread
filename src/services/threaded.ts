...
export class ThreadedServiceRegistry extends AbstractThreadedServiceRegistry {
  ...
  setMaxWorkersByCpu() {
    ...
    const evenCpuCycles = cpuStat
      .filter((_cpu, i) => i % 2 === 0)
      .reduce((acc, cpu) => acc + cpu.times.user + cpu.times.sys, 0);
    const oddCpuCycles = cpuStat
      .filter((_cpu, i) => i % 2 === 1)
      .reduce((acc, cpu) => acc + cpu.times.user + cpu.times.sys, 0);

    const isLikelyHyperThreaded = oddCpuCycles / evenCpuCycles < 0.5;

    this.maxWorkers = isLikelyHyperThreaded
      ? cpuStat.length / 2
      : cpuStat.length;
  }
...
export const { Method, Param, Ctx, RPCReflect, Threaded } =
  instance.decorators();
