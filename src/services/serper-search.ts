...
export class SerperSearchService extends AsyncService {
  ...
  override async init() {
    ...
    this.serperGoogleSearchHTTP = new SerperGoogleHTTP(
      this.secretExposer.SERPER_SEARCH_API_KEY,
    );
    this.serperBingSearchHTTP = new SerperBingHTTP(
      this.secretExposer.SERPER_SEARCH_API_KEY,
    );
  }
  ...
  async doSearch(
    ...
    while (maxTries--) {
      ...
      try {
        ...
        this.logger.debug(`External search took ${dt}ms`, {
          searchDt: dt,
          variant,
        });
        ...
        const dt = Date.now() - t0;
        this.logger.error(`${variant} search failed: ${err?.message}`, {
          searchDt: dt,
