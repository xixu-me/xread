...
export class RobotsTxtService extends AsyncService {
  ...
  async getCachedRobotTxt(origin: string) {
    ...
    let buff;
    buff = await this.firebaseStorageBucketControl
      .downloadFile(cacheLoc)
      .catch(() => undefined);
    if (buff) {
    ...
    if (!r.ok) {
      throw new DownstreamServiceFailureError(
        `Failed to fetch robots.txt from ${origin}: ${r.status} ${r.statusText}`,
      );
    }
    buff = Buffer.from(await r.arrayBuffer());

    this.firebaseStorageBucketControl
      .saveFile(cacheLoc, buff, {
        contentType: "text/plain",
      })
      .catch((err) => {
        this.logger.warn(`Failed to save robots.txt to cache: ${err}`, {
  ...
  async assertAccessAllowed(url: URL, inputMyUa = "*") {
    ...
    try {
      ...
      throw new AssertionFailureError(
        `Failed to load robots.txt from ${url.origin}: ${err}`,
      );
    }
        ...
        if (value.includes("*")) {
          ...
          if (url.pathname.startsWith(head) && url.pathname.endsWith(tail)) {
            throw new ResourcePolicyDenyError(
              `Access to ${url.href} is disallowed by site robots.txt: For ${uaLine}, ${line}`,
            );
          }
