...
export class GeoIPService extends AsyncService {
  ...
  async _lazyload() {
    ...
    try {
      ...
      if (
        ...
        this.logger.warn(
          `GeoIP database is missing at ${mmdpPath}; GeoIP lookups are disabled.`,
        );
        return;
      }
      throw error;
  ...
  async lookupCity(
    ip: string,
    lang: GEOIP_SUPPORTED_LANGUAGES = GEOIP_SUPPORTED_LANGUAGES.EN,
  ) {
  ...
  async lookupCities(
    ...
    const r = (
      await Promise.all(ips.map((ip) => this.lookupCity(ip, lang)))
    ).filter(Boolean) as GeoIPCityResponse[];
