...
export class AltTextService extends AsyncService {
  ...
  async caption(url: string) {
    try {
      ...
      const svgHint = contentTypeHint.includes("svg")
        ? `Beware this image is a SVG rendered on a gray background, the gray background is not part of the image.\n\n`
        : "";
      const svgSystemHint = contentTypeHint.includes("svg")
        ? ` Sometimes the system renders SVG on a gray background. When this happens, you must not include the gray background in the description.`
        : "";

      const r = await this.imageInterrogator.interrogate(
      ...
      throw new AssertionFailureError({
        message: `Could not generate alt text for url ${url}`,
        cause: err,
      });
    }
  }
  ...
  async getAltText(imgBrief: ImgBrief) {
    ...
    if (
      imgBrief.alt &&
      !this.altsToIgnore.includes(imgBrief.alt.trim().toLowerCase())
    ) {
      return imgBrief.alt;
    }
    ...
    try {
      ...
      this.logger.warn(`Unable to generate alt text for ${imgBrief.src}`, {
        err,
      });
    }
    ...
    const expireMixin = generatedCaption
      ? {}
      : { expireAt: new Date(Date.now() + 1000 * 3600 * 24) };
