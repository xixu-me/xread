    ...
    try {
      ...
      if (err instanceof HTTPServiceError) {
        if (err.status === 429) {
          ...
          this.logger.warn("Cloudflare browser rendering rate limit exceeded", {
            url,
          });

          throw new ServiceNodeResourceDrainError(
            `Cloudflare browser rendering (our account) is at capacity, please try again later or switch to another engine.`,
          );
        }
      }
