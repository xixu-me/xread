...
export class BlackHoleDetector extends AsyncService {
  ...
  async routine() {
    ...
    if (
      ...
      this.lastIncomingRequestTs &&
      lastWorked &&
      this.lastIncomingRequestTs >= lastWorked &&
      dt > this.maxDelay * (this.strikes + 1)
    ) {
      this.logger.warn(
        `BlackHole detected, last worked: ${Math.ceil(dt / 1000)}s ago, concurrentRequests: ${this.concurrentRequests}`,
      );
      this.strikes += 1;
    }
    ...
    if (this.strikes >= 3) {
      this.logger.error(
        `BlackHole detected for ${this.strikes} strikes, last worked: ${Math.ceil(dt / 1000)}s ago, concurrentRequests: ${this.concurrentRequests}`,
      );
      process.nextTick(() => {
        this.emit(
          "error",
