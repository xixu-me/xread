...
} from "../shared/3rd-party/brave-search";
...
export class BraveSearchService extends AsyncService {
  ...
  override async init() {
    ...
    this.braveSearchHTTP = new BraveSearchHTTP(
      this.secretExposer.BRAVE_SEARCH_API_KEY,
    );
  }
  ...
  async webSearch(query: WebSearchQueryParams) {
    ...
    if (ip) {
      const geoip = await this.geoipControl.lookupCity(
        ip,
        GEOIP_SUPPORTED_LANGUAGES.EN,
      );
      ...
      if (geoip?.subdivisions?.length) {
        extraHeaders["X-Loc-State"] = encodeURIComponent(
          `${geoip.subdivisions[0].code}`,
        );
        extraHeaders["X-Loc-State-Name"] = encodeURIComponent(
    ...
    if (encoded.q) {
      encoded.q =
        Buffer.from(encoded.q).toString("ascii") === encoded.q
          ? encoded.q
          : encodeURIComponent(encoded.q);
    }
    ...
    while (maxTries--) {
      try {
        const r = await this.braveSearchHTTP.webSearch(encoded, {
...
export class BraveSearchExplicitOperatorsDto extends AutoCastable {
  ...
  addTo(searchTerm: string) {
    ...
    const opPart =
      chunks.length > 1 ? chunks.map((x) => `(${x})`).join(" AND ") : chunks;
