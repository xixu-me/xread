...
export class JSDomControl extends AsyncService {
  ...
  async narrowSnapshot(
    ...
    try {
      ...
      return await this.actualNarrowSnapshot(snapshot, {
        ...options,
        sideLoad: undefined,
      });
    } catch (err: any) {
      ...
      throw new AssertionFailureError(
        `Failed to process the page: ${err?.message}`,
      );
    }
  }

  @Threaded()
  async actualNarrowSnapshot(
    ...
    if (!jsdom.window.document.documentElement) {
      jsdom = this.linkedom.parseHTML(
        `<html><body>${sourceHTML}</body></html>`,
      );
    }
    const allNodes: Node[] = [];
    ...
    if (options?.withIframe) {
      jsdom.window.document
        .querySelectorAll("iframe[src],frame[src]")
        .forEach((x) => {
          const src = x.getAttribute("src");
          const thisSnapshot = snapshot.childFrames?.find(
            (f) => f.href === src,
          );
          if (options?.withIframe === "quoted") {
            const blockquoteElem =
              jsdom.window.document.createElement("blockquote");
    ...
    if (Array.isArray(options?.removeSelector)) {
      ...
      jsdom.window.document
        .querySelectorAll(options.removeSelector)
        .forEach((x) => x.remove());
    }
    ...
    if (Array.isArray(options?.targetSelector)) {
      ...
      for (const x of options!.targetSelector.map((x) =>
        jsdom.window.document.querySelectorAll(x),
      )) {
        x.forEach((el) => {
      ...
      bewareTargetContentDoesNotExist = true;
      jsdom.window.document
        .querySelectorAll(options.targetSelector)
        .forEach((el) => {
          if (!allNodes.includes(el)) {
            allNodes.push(el);
          }
    ...
    if (
      allNodes.length === 1 &&
      allNodes[0].nodeName === "#document" &&
      (allNodes[0] as any).documentElement
    ) {
      ...
      rootDoc = this.linkedom.parseHTML("<html><body></body></html>").window
        .document;
    ...
    const r = {
      ...
      imgs:
        (snapshot.imgs || rebuiltImgs)?.filter((x) => imgSet.has(x.src)) || [],
    } as PageSnapshot;

    const dt = Date.now() - t0;
    if (dt > 1000) {
      this.logger.warn(`Performance issue: Narrowing snapshot took ${dt}ms`, {
        url: snapshot.href,
        dt,
      });
    }
  ...
  async inferSnapshot(snapshot: PageSnapshot) {
    ...
    try {
      ...
      const links = Array.from(
        jsdom.window.document.querySelectorAll("a[href]"),
      )
        .map((x: any) => [
      ...
      const imgs = Array.from(
        jsdom.window.document.querySelectorAll("img[src],img[data-src]"),
      ).map((x: any) => {
    ...
    const dt = Date.now() - t0;
    if (dt > 1000) {
      this.logger.warn(`Performance issue: Inferring snapshot took ${dt}ms`, {
        url: snapshot.href,
        dt,
      });
    }
  ...
  cleanRedundantEmptyLines(text: string) {
    ...
    return lines
      .filter((_line, i) => mappedFlag[i] || mappedFlag[i - 1])
      .join("\n");
  }
  ...
  async cleanHTMLforLMs(
    ...
    const final = this.cleanRedundantEmptyLines(
    ...
    if (dt > 1000) {
      this.logger.warn(
        `Performance issue: Cleaning HTML for LMs took ${dt}ms`,
        { dt },
      );
    }
  ...
  snippetToElement(snippet?: string, _url?: string) {
    const parsed = this.linkedom.parseHTML(
      snippet || "<html><body></body></html>",
    );

    // Hack for turndown gfm table plugin.
    parsed.window.document.querySelectorAll("table").forEach((x) => {
      Object.defineProperty(x, "rows", {
        value: Array.from(x.querySelectorAll("tr")),
        enumerable: true,
      });
    });
  ...
  runTurndown(
    ...
    html: TurndownService.Node | string,
  ) {
    const t0 = Date.now();
