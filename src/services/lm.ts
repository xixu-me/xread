...
export class LmControl extends AsyncService {
  ...
  async *geminiFromBrowserSnapshot(
    ...
    if (!pageshot) {
      throw new AssertionFailureError(
        "Screenshot of the page is not available",
      );
    }

    const html = await this.jsdomControl.cleanHTMLforLMs(
      snapshot.html,
      "script,link,style,textarea,select>option,svg",
    );
    ...
    const it = this.commonLLM.iterRun("vertex-gemini-1.5-flash-002", {
      ...
      options: {
        system:
          "You are XreadLM-v7, a model that generates Markdown source files only. No HTML, notes and chit-chats allowed",
        stream: true,
      },
    });
  ...
  async *xreadLMFromSnapshot(
    schema?: string,
    instruction: string = "Infer useful information from the HTML and present it in a structured JSON object.",
    snapshot?: PageSnapshot,
  ) {
    if (!snapshot) {
      throw new AssertionFailureError("Snapshot of the page is not available");
