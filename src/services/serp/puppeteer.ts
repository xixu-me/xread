...
export class SERPSpecializedPuppeteerControl extends AsyncService {
  ...
  async newPage<T>(bewareDeadLock: any = false) {
    ...
    preparations.push(
      page.exposeFunction(this._REPORT_FUNCTION_NAME, (thing: T) => {
  ...
  async controlledScrap<T>(
    parsedUrl: URL,
    func: (this: void) => Promise<T>,
...
func().then((result) => {
    ...
    const gotoPromise = page.goto(url, goToOptions).catch((err) => {
    ...
    const controlTimeout = setTimeout(() => {
      resultDeferred.reject(
        new TimeoutError(`Control function did not respond in time`),
      );
    }, timeout);
