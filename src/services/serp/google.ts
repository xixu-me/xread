...
export class GoogleSERP extends AsyncService {
  ...
  constructor(
    ...
    const filteredDeps = isMainThread
      ? arguments
      : _.without(arguments, puppeteerControl);
    super(...filteredDeps);
  }
  ...
  async sideLoadWithAllocatedProxy(url: URL, opts?: ScrappingOptions) {
    ...
    if (!this.proxyProvider.isConfigured()) {
      this.logger.info(
        `No proxy configured, falling back to direct Google request`,
        { url: url.href },
      );
      return this.curlControl.sideLoad(url, opts);
    }
    ...
    if (r.status === 429) {
      throw new ServiceBadAttemptError(
        "Google returned a 429 error. This may happen due to various reasons, including rate limiting or other issues.",
      );
    }
  ...
  async webSearch(query: { [k: string]: any }, opts?: ScrappingOptions) {
    ...
    const snapshot = await this.puppeteerControl.controlledScrap(
      url,
      getWebSearchResults,
      opts,
    );
  ...
  async imageSearch(query: { [k: string]: any }, opts?: ScrappingOptions) {
    ...
    url.searchParams.set(
      "async",
      `_fmt:json,p:1,ijn:${query.start ? Math.floor(query.start / (query.num || 10)) : 0}`,
    );
    ...
    if (sideLoaded.status !== 200 || !sideLoaded.file) {
      throw new ServiceBadAttemptError(
        "Google returned an error page. This may happen due to various reasons, including rate limiting or other issues.",
      );
    }
...
async function getWebSearchResults() {
  ...
  // @ts-ignore
  await Promise.race([
    window.waitForSelector('div[data-async-context^="query"]'),
    window.waitForSelector("#botstuff .mnr-c"),
  ]);
  ...
  const query = decodeURIComponent(
    wrapper1.getAttribute("data-async-context")?.split("query:")[1] || "",
  );
  ...
  const candidates = Array.from(
    wrapper1.querySelectorAll("div[lang],div[data-surl]"),
  );
  ...
  return candidates
    .map((x) => {
      ...
      const source = Array.from(primaryLink.querySelectorAll("span")).find(
        (x) => x.textContent,
      )?.textContent;
      const cite = primaryLink.querySelector("cite[role=text]")?.textContent;
      let date = cite?.split("·")[1]?.trim();
      const snippets = Array.from(
        x.querySelectorAll('div[data-sncf*="1"] span'),
      );
      let snippet: string | null | undefined =
        snippets[snippets.length - 1]?.textContent;
      if (!snippet) {
        snippet = x.querySelector("div.IsZvec")?.textContent?.trim() || null;
      }
      date ??= snippets[snippets.length - 2]?.textContent?.trim();
      const imageUrl = x
        .querySelector('div[data-sncf*="1"] img[src]:not(img[src^="data"])')
        ?.getAttribute("src");
      let siteLinks = Array.from(
        x.querySelectorAll('div[data-sncf*="3"] a[href]'),
      )
        .map((l) => {
...
async function getNewsSearchResults() {
  ...
  const candidates = Array.from(
    wrapper1.querySelectorAll("div[data-news-doc-id]"),
  );
  ...
  return candidates
    .map((x) => {
      ...
      const innerSpans = Array.from(
        titleElem.parentElement?.querySelectorAll("span") || [],
      );
