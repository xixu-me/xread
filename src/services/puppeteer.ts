...
const mediaResourceTypes = new Set(["stylesheet", "image", "font", "media"]);
...
export class PuppeteerControl extends AsyncService {
    ...
    this.on("crippled", () => {
      ...
      if (crippledTimes > 5) {
        process.nextTick(() => {
          this.emit(
            "error",
            new Error("Browser crashed too many times, quitting..."),
          );
          // process.exit(1);
        });
      }
  ...
  override async init() {
    ...
    this.logger.info(
      `Browser launched: ${this.browser.process()?.pid}, ${this.ua}`,
    );
    this.effectiveUA = this.ua
      .replace(/Headless/i, "")
      .replace(
    ...
    await this.newPage("beware_deadlock").then((r) =>
      this.__loadedPage.push(r),
    );
  ...
  async newPage(bewareDeadLock: any = false) {
    ...
    try {
      ...
      throw new ServiceNodeResourceDrainError(
        `This specific worker node failed to open a new page, try again.`,
      );
    }
    const preparations = [];
    ...
    preparations.push(
      page.exposeFunction("setViewport", (viewport: Viewport | null) => {
    ...
    page.on("request", async (req) => {
      ...
      if (this.circuitBreakerHosts.has(parsedUrl.hostname.toLowerCase())) {
        page.emit("abuse", {
          ...
          page,
          sn,
          reason: `Abusive request: ${requestUrl}`,
        });
        return req.abort("blockedbyclient", 1000);
      }
      ...
      if (
        ...
        page.emit("abuse", {
          ...
          page,
          sn,
          reason: `Suspicious action: Request to localhost: ${requestUrl}`,
        });
      ...
      if (reqCounter > 1000) {
        if (rps > 60 || reqCounter > 2000) {
          page.emit("abuse", {
            ...
            page,
            sn,
            reason: `DDoS attack suspected: Too many requests`,
          });
          halt = true;
      ...
      if (domainSet.size > 200) {
        page.emit("abuse", {
          ...
          page,
          sn,
          reason: `DDoS attack suspected: Too many domains`,
        });
        halt = true;
...
`);
  ...
  async getNextPage() {
      ...
      if (this.__loadedPage.length <= 1) {
        process.nextTick(() => {
          this.newPage()
            ...
            .catch((err) => {
              this.logger.warn(`Failed to load new page ahead of time`, {
                err,
              });
            });
        });
    ...
    const timer = setTimeout(() => {
      this.logger.warn(
        `Page is not allowed to live past 5 minutes, ditching page ${this.snMap.get(thePage!)}...`,
      );
  ...
  async *scrap(
    ...
    page.on("request", async (req) => {
      ...
      const proxy =
        options.proxyUrl || sideload?.proxyOrigin?.[reqUrlParsed.origin];
      ...
      if (proxy && ctx) {
        return await this.asyncLocalContext.bridge(ctx, async () => {
          try {
            ...
            this.logger.warn(
              `Failed to sideload browser request ${reqUrlParsed.origin}`,
    ...
    if (options.injectPageScripts?.length) {
      page.on("framenavigated", (frame) => {
        ...
        pageScriptEvaluations.push(
          Promise.allSettled(
            options.injectPageScripts!.map((x) =>
              frame.evaluate(x).catch((err) => {
                this.logger.warn(`Error in evaluation of page scripts`, {
                  err,
                });
              }),
            ),
    ...
    if (options.injectFrameScripts?.length) {
      page.on("framenavigated", (frame) => {
        frameScriptEvaluations.push(
          Promise.allSettled(
            options.injectFrameScripts!.map((x) =>
              frame.evaluate(x).catch((err) => {
                this.logger.warn(`Error in evaluation of frame scripts`, {
                  err,
                });
              }),
            ),
    ...
    if (options.cookies) {
      const mapped = options.cookies.map((x) => {
        const draft: CookieParam = {
          ...
          path: x.path,
          expires: x.expires
            ? Math.floor(x.expires.valueOf() / 1000)
            : undefined,
    ...
    const crippleListener = () =>
      nextSnapshotDeferred.reject(
        new ServiceCrashedError({ message: `Browser crashed, try again` }),
      );
    ...
    const doFinalization = async () => {
      ...
      try {
        ...
        pageshot =
          (await this.takeScreenShot(page, { fullPage: true })) || pageshot;
        if (snapshot) {
      ...
      this.logger.info(`Page ${sn}: Snapshot of ${url} done`, {
        url,
    ...
    const gotoPromise = page
      ...
      .then(async (stuff) => {
        ...
        await Promise.race([
          Promise.allSettled([
            ...pageScriptEvaluations,
            ...frameScriptEvaluations,
          ]),
          delayPromise,
        ]).catch(() => void 0);
        return stuff;
    ...
    if (options.waitForSelector) {
      ...
      waitForPromise = nextSnapshotDeferred.promise.then(() => {
        ...
        const p = (
          Array.isArray(options.waitForSelector)
            ? Promise.all(
                options.waitForSelector.map((x) =>
                  page.waitForSelector(x, { timeout: thisTimeout }),
                ),
              )
          ...
          .catch((err) => {
            ...
            this.logger.warn(
              `Page ${sn}: Failed to wait for selector ${options.waitForSelector}`,
              { err },
            );
          });
        return p as any;
      });
      finalizationPromise = Promise.allSettled([
        waitForPromise,
        gotoPromise,
      ]).then(doFinalization);
    } else {
    ...
    try {
      ...
      while (true) {
        ...
        if (
          options.favorScreenshot &&
          snapshot?.title &&
          snapshot?.html !== lastHTML
        ) {
          screenshot = (await this.takeScreenShot(page)) || screenshot;
          pageshot =
            (await this.takeScreenShot(page, { fullPage: true })) || pageshot;
  ...
  protected async takeScreenShot(
    page: Page,
    opts?: Parameters<typeof page.screenshot>[0],
    ...
    const r = (await Promise.all(
      childFrames.map(async (x) => {
        ...
        try {
          ...
          this.logger.warn(`Failed to snapshot child frame ${thisUrl}`, {
            err,
          });
          return undefined;
        }
