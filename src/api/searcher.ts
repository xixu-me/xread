...
import {
  SerperBingSearchService,
  SerperGoogleSearchService,
} from "../services/serp/serper";
...
export class SearcherHost extends RPCHost {
  ...
  async search(
    ...
    num?: number,
    @Param("gl", { validate: (v: string) => /^[a-z]{2}$/i.test(v || "") })
    gl?: string,
    @Param("hl", { validate: (v: string) => /^[a-z]{2}$/i.test(v || "") })
    hl?: string,
    ...
    const crawlWithoutContent =
      crawlerOptions.respondWith.includes("no-content");
    ...
    if (highFreqKey?.blockedUntil) {
      const now = new Date();
      const blockedTimeRemaining =
        highFreqKey.blockedUntil.valueOf() - now.valueOf();
    ...
    rpcReflect.finally(async () => {
      if (chargeAmount) {
        if (uid) {
          auth
            .reportUsage(chargeAmount, `xread-${rpcReflect.name}`)
            .catch((err) => {
    ...
    if (!results.length) {
      throw new AssertionFailureError(
        `No search results available for query ${searchQuery}`,
      );
    }
    ...
    const trimmedResults: any[] = results
      .filter((x) => Boolean(x.link))
      .slice(0, targetResultCount)
      .map((x) => this.mapToFinalResults(x));
    trimmedResults.toString = function () {
      let r =
        this.map((x, i) => (x ? Reflect.apply(x.toString, x, [i]) : ""))
          .join("\n\n")
          .trimEnd() + "\n";
      if (fallbackQuery) {
    ...
    if (
      !crawlerOptions.respondWith.includes("no-content") &&
      ["html", "text", "shot", "markdown", "content"].some((x) =>
        crawlerOptions.respondWith.includes(x),
      )
    ) {
    ...
    if (!ctx.accepts("text/plain") && ctx.accepts("text/event-stream")) {
      ...
      try {
        ...
        this.logger.error(
          `Failed to collect search result for query ${searchQuery}`,
          { err: marshalErrorLike(err) },
        );
        sseStream.write({
          event: "error",
    ...
    if (
      ...
      if (!lastScrapped) {
        throw new AssertionFailureError(
          `No content available for query ${searchQuery}`,
        );
      }
    ...
    for await (const scrapped of it) {
      ...
      return assignTransferProtocolMeta(`${scrapped}`, {
        contentType: "text/plain",
        envelope: null,
      });
    }
  ...
  async searchWithFallback(
    params: SerperSearchQueryParams & {
      variant: "web" | "images" | "news";
      provider?: string;
    },
    ...
    const lastResort = containsRTL
      ? queryTerms.slice(queryTerms.length - 2)
      : queryTerms.slice(0, 2);

    this.logger.info(
      `No results for "${originalQuery}", trying fallback queries`,
    );
    ...
    while (tryTimes < n) {
      ...
      terms = containsRTL
        ? queryTerms.slice(delta)
        : queryTerms.slice(0, queryTerms.length - delta);
      ...
      const fallbackResults = await this.cachedSearch(
        params.variant,
        fallbackParams,
        noCache,
      );
  ...
  async *fetchSearchResults(
    ...
    for await (const scrapped of this.crawler.scrapMany(
      urls,
      options,
      crawlerOptions,
    )) {
      const mapped = scrapped.map((x, i) => {
        ...
        return this.crawler
          .formatSnapshotWithPDFSideLoad(mode, x, urls[i], undefined, options)
          .then((r) => {
            snapshotMap.set(x, r);
          ...
          .catch((err) => {
            this.logger.error(`Failed to format snapshot for ${urls[i].href}`, {
              err: marshalErrorLike(err),
            });
  ...
  reOrganizeSearchResults(searchResults: FormattedPage[], count?: number) {
    ...
    const [qualifiedPages, unqualifiedPages] = _.partition(searchResults, (x) =>
      this.pageQualified(x),
    );
  ...
  pageQualified(formattedPage: FormattedPage) {
    return (
      (formattedPage.title && formattedPage.content) ||
  ...
  searchResultsQualified(
    results: FormattedPage[],
    targetResultCount = this.targetResultCount,
  ) {
    return (
      _.every(results, (x) => this.pageQualified(x)) &&
      results.length >= targetResultCount
    );
  }
  ...
  async getFavicon(domain: string) {
    ...
    try {
      ...
      this.logger.warn(`Failed to get favicon base64 string`, {
        err: marshalErrorLike(error),
      });
      return "";
    }
  }
  ...
  *iterProviders(preference?: string, _variant?: string) {
  ...
  async cachedSearch(
    ...
    if (!noCache) {
      cache = (
        await SERPResult.fromFirestoreQuery(
      ...
      if (cache) {
        ...
        const stale =
          cache.createdAt.valueOf() < Date.now() - this.cacheValidMs;
        this.logger.info(
          `${stale ? "Stale cache exists" : "Cache hit"} for search query "${query.q}", normalized digest: ${queryDigest}, ${age}ms old`,
          {
            query,
            digest: queryDigest,
            age,
    ...
    try {
      ...
      outerLoop: for (const client of this.iterProviders(provider, variant)) {
        ...
        try {
          ...
          this.logger.info(
            `Search took ${dt}ms, ${client.constructor.name}(${variant})`,
            { searchDt: dt, variant, client: client.constructor.name },
          );
          break outerLoop;
        } catch (err) {
          lastError = err;
          const dt = Date.now() - t0;
          this.logger.warn(
            `Failed to do ${variant} search using ${client.constructor.name}`,
            { err, variant, searchDt: dt },
          );
        }
      }
      ...
      if (cache) {
        this.logger.warn(
          `Failed to fetch search result, but a stale cache is available. falling back to stale cache`,
          { err: marshalErrorLike(err) },
        );
...
const searchResultProto = {
  toString(this: FormattedPage, i?: number) {
    ...
    if (this.images) {
      const imageSummaryChunks = [
        `${i === undefined ? "" : `[${i + 1}] `}Images:`,
      ];
