...
function derivePublicOrigins(ctx: Context) {
  ...
  const currentUrl = new URL(ctx.origin);
...
export class SerpHost extends RPCHost {
  ...
  async getIndex(ctx: Context, auth?: AuthDTO) {
    const { searchOrigin, homepage } = derivePublicOrigins(ctx);
    const indexObject: Record<string, string | number | undefined> =
      Object.create(indexProto);
    Object.assign(indexObject, {
      textSearch: `${searchOrigin}/search?q=YOUR_SEARCH_QUERY`,
      jsonSearch: `${searchOrigin}/?q=YOUR_SEARCH_QUERY`,
      repository: homepage,
    });
  ...
  async search(
    ...
    @Param("hl", {
      validate: (v: string) =>
        WORLD_LANGUAGES.some((l) => l.code === v) ||
        /^[a-z]{2}$/i.test(v || ""),
    })
    _hl?: string,
    ...
    if (highFreqKey?.blockedUntil) {
      ...
      if (blockedTimeRemaining > 0) {
        this.logger.warn(
          `Rate limit triggered for ${uid}, this request should have been blocked`,
        );
    ...
    let results = await this.cachedSearch(
      variant,
      {
    ...
    if (fallback && !results?.length && (!page || page === 1)) {
      ...
      while (tryTimes < n) {
        ...
        results = await this.cachedSearch(
          variant,
          {
            provider: searchEngine,
            q: realQuery,
    ...
    try {
      ...
      outerLoop: for (const client of this.iterProviders(provider, variant)) {
        ...
        try {
          switch (variant) {
            case "images": {
              r = await Reflect.apply(client.imageSearch, client, [
                query,
                scrappingOptions,
              ]);
              break;
            ...
            case "news": {
              r = await Reflect.apply(client.newsSearch, client, [
                query,
                scrappingOptions,
              ]);
              break;
            ...
            case "web":
            default: {
              r = await Reflect.apply(client.webSearch, client, [
                query,
                scrappingOptions,
              ]);
              break;
